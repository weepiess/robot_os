#ifndef BASIC_BLACKBOARD_H
#define BASIC_BLACKBOARD_H
...
class BasicBlackboard
{
    ...
    BasicBlackboard(std::unique_ptr<BlackboardImpl> base) : impl_(std::move(base))
    {
    }

  public:
    typedef std::shared_ptr<BasicBlackboard> Ptr;

    BasicBlackboard() = delete;
    ...
    */
    ...
    static BasicBlackboard::Ptr create(Args... args)
    {
        std::unique_ptr<BlackboardImpl> base(new ImplClass(args...));
        return std::shared_ptr<BasicBlackboard>(new BasicBlackboard(std::move(base)));
    }

    virtual ~BasicBlackboard() = default;
